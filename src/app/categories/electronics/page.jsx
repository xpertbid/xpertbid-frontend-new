...
export default function ElectronicsCategoryPage() {
  ...
  useEffect(() => {
    const fetchElectronicsProducts = async () => {
      try {
        ...
        if (response.success) {
          ...
          const electronicsProducts = response.data.filter(product => 
            ...
            product.tags?.some(tag => 
              ['electronics', 'electronic', 'tech', 'technology', 'gadget', 'device'].includes(tag.toLowerCase())
            )
          );
        ...
        setError('Error loading electronics products');
  ...
  const getFirstImage = (images) => {
    if (!images) return 'https://images.unsplash.com/photo-1498049794561-7780e7231661?w=400&h=300&fit=crop';
    if (Array.isArray(images)) return images[0] || 'https://images.unsplash.com/photo-1498049794561-7780e7231661?w=400&h=300&fit=crop';
    if (typeof images === 'string') {
      try {
        const parsed = JSON.parse(images);
        return Array.isArray(parsed) && parsed.length > 0 ? parsed[0] : 'https://images.unsplash.com/photo-1498049794561-7780e7231661?w=400&h=300&fit=crop';
      } catch {
        return images;
      }
    }
      ...
      <Layout>
        <div className="container py-5">
          <div className="text-center">
            ...
            <p className="mt-3">Loading electronics products...</p>
          </div>
        </div>
      </Layout>
    );
      ...
      <div className="container py-5">
        <div className="row">
          <div className="col-12">
            ...
            <p className="text-muted mb-5">Discover the latest electronic devices and gadgets.</p>
          </div>
        </div>
        ...
        {products.length > 0 ? (
          ...
          <div className="row">
            <div className="col-12 text-center py-5">
              ...
              <h4 className="text-muted">No electronics products available</h4>
              <p className="text-muted">Check back later for new electronics products.</p>
